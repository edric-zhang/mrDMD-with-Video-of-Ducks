...
while len(frames_list) < max_frames:
    ...
    # 1. Convert directly from BGR to GRAYSCALE (Saves massive memory)
    gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    
    # 2. Resize to a higher resolution (e.g., 320x180)
    # This gives crisp lines without a massive file footprint
    resized_frame = cv2.resize(gray_frame, (320, 180))
    
    # 3. Flatten the grayscale frame
